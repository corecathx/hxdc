            ...
            if (state != State.Closed) {
                ...
                if (onclose != null) {
                    ...
                    haxe.EntryPoint.runInMainThread(()->{
                        if (_buffer.chunks[0].toString().contains("Authentication failed")) {
                            throw "Discord gateway returned a non-reconnect error\n"+_buffer.chunks[0].toString().substring(4) + "\n\nThe account token sent with your identify payload is incorrect.";
                        }
                        if (_buffer.chunks[0].toString().contains("Sharding required")) {
                            throw "Discord gateway returned a non-reconnect error\n"+_buffer.chunks[0].toString().substring(4) + "\n\nThe session would have handled too many guilds - you are required to shard your connection in order to connect.";
                        }
                        if (_buffer.chunks[0].toString().contains("Invalid API version")) {
                            throw "Discord gateway returned a non-reconnect error\n"+_buffer.chunks[0].toString().substring(4) + "\n\nYou sent an invalid version for the gateway.";
                        }
                        if (_buffer.chunks[0].toString().contains("Invalid intent(s)")) {
                            throw "Discord gateway returned a non-reconnect error\n"+_buffer.chunks[0].toString().substring(4) + "\n\nYou sent an invalid intent for a Gateway Intent. You may have incorrectly calculated the bitwise value.";
                        }
                        if (_buffer.chunks[0].toString().contains("Disallowed intent(s)")) {
